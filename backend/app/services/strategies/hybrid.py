...
@dataclass
class HybridSignal:
    """Signal generated by Hybrid strategy."""
...
class HybridStrategy:
    ...
    def generate_signal(self, df: pd.DataFrame) -> HybridSignal:
        """
        Genera señal de trading basada en el régimen actual.

        Returns:
            HybridSignal con direction, stop_loss, take_profit, etc.
        """
        if len(df) < max(self.ema_trend_period, self.breakout_range_period) + 10:
            return HybridSignal(direction="WAIT", reason="Insufficient data")
        ...
        if pd.isna(current['adx']) or pd.isna(current['atr']):
            return HybridSignal(direction="WAIT", reason="Missing indicators")

        regime = self.get_regime(current['adx'])

        if regime == "consolidation":
            signal = self._breakout_signal(current, prev)
        else:
            signal = self._macd_signal(current, prev)
    ...
    def _breakout_signal(self, current: pd.Series, prev: pd.Series) -> Optional[HybridSignal]:
    ...
    def _macd_signal(self, current: pd.Series, prev: pd.Series) -> Optional[HybridSignal]:
